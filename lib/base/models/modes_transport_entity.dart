...
extension ModesTransportTypeExtension on ModesTransportType {
  PlanEntity? getPlanEntity(
    ModesTransportEntity? modesTransport,
  ) {
    PlanEntity? currentPlan;
    switch (this) {
      case ModesTransportType.walkPlan:
        currentPlan = modesTransport?.walkPlan;
        break;
      case ModesTransportType.bikePlan:
        currentPlan = modesTransport?.bikePlan;
        break;
      case ModesTransportType.bikeAndVehicle:
        currentPlan = modesTransport?.bikeAndVehicle;
        break;
      case ModesTransportType.carAndCarPark:
        currentPlan = modesTransport?.carAndCarPark;
        break;
      case ModesTransportType.parkRidePlan:
        currentPlan = modesTransport?.parkRidePlan;
        break;
...
class ModesTransportEntity extends Equatable {
  ...
  Widget getIconBikePublic() {
    final publicModes =
        filterOnlyBikeAndWalk(bikeAndVehicle?.itineraries ?? [])[0]
            .legs
            .where(
              (element) =>
